...
1. В правила добавлен новый параметр «Заполнять без подтверждения». Он позволяет автоматически устанавливать реквизиты документа без запроса подтверждения

2. Теперь, при условии установки параметра «Заполнять без подтверждения», автоправила будут применяться ко всем документам конфигурации, для которых существуют правила, а не только для тех, в которых добавлены реквизиты P&L

[image:./reliz-1-45-0-13.png:::0,0,100,100::square,0.9285,74.538,57.4745,22.7926,,top-left:1077px:487px:center]
...
3. Оптимизировано получение данных для списка «Документы», что ускорило открытие и формирование списка.
...
1. В документе «Учет основных средств»  во вкладке «Предварительный просмотр» добавлена группировка помесячно, если в таблице введены разными числами одного месяца даты ввода в эксплуатацию.

2. В документе «Отражение заработной платы в УУ» добавлена команда «Установить параметры», которая позволяет выбрать реквизиты P&L (статью,  доп. аналитику, проект, раздел проекта) и заполнить выбранными значениями все выделенные строки в таблице вкладки  «Основное» или «Налоги, взносы и прочее».

   [image:./reliz-1-45-0-4.png:::0,0,100,100::square,55.7447,11.2412,17.0821,13.1148,,top-left:1645px:427px:center]

3. Для документов «Кредиты и займы» добавлена возможность помечать документ на удаление, даже если это недействующая версия. Если документ помечается на удаление он автоматически убирается из цепочки версий документов, ни является ничьим основанием и ни формирует никаких движений.

4. В список документов «Кредиты и займы» добавлен блок под названием «Платежный календарь». Колонка «Общая сумма» рассчитывается как сумма всех запланированных платежей по выбранному документу в рамках платежного календаря.
...
2. Для **1С:Бухгалтерия** **предприятия**  в документах «Ведомость на выплату зарплаты» и «Уведомление об исчисленных суммах налогов» добавлена возможность на основании документа создать планируемый платеж (списание).

3. В форме выбора платежа из денежного документа расширен период поиска, чтобы видеть все оплаченные текущим документом платежи.

4. Для контроля лимитов добавлена настройка версии финансового плана. Настройка выполняется в разделе «Платежный календарь»

   ![](./reliz-1-45-0-10.png){width=1126px height=106px}

5. Изменено появление плашки «Контроль лимитов». Теперь она появляется во всех вариантах отображения формы «Платежный календарь»

6. В отчете платежного календаря для варианта отчета «Платежный календарь + БДДС» добавлен отбор по версии финансового плана.
...
7. В документе «Реестр платежей» добавлено заполнение колонки «Контрагент» физическим лицом, если оно указано в запланированных платежах.

8. Также добавлена колонка «Комментарий», в которую можно вносить текст. В печатной форме «Реестр платежей» комментариями заполняется колонка «Примечание». Данные комментарии передаются и в заявки на согласование, в поле «Описание»

9. Для **1С:Бухгалтерия** **предприятия**  добавлен отбор «Остатки по депозитам» (по умолчанию отбор установлен как «Не включать остатки по депозитам»).
...
10. Доработан функционал «Разбивки платежа» в модуле платежного календаря системы:
...
1. Добавлено очищение прикрепленного файла при копировании планируемого платежа.
...
3. Отчет «Бюджет + финансовый поток». Теперь выводятся сначала доходы, потом расходы.
...
1. Добавлен расчет колонки % от дохода для статей отчета

   [image:./reliz-1-45-0-12.png:::0,0,100,100::square,76.4468,77.2989,13.6574,7.9023,,top-left&square,47.6273,34.7701,10.4167,65.2299,,top-left:2041px:822px:center]

2. В варианте отчета «P&L: План-Факт»  для статей, по которым в выбранной плановой версии БДР не установлено плановое значение (фактически 0), отклонение  заполняется фактическим значением. Также было изменено наименование колонки с «% отклонения» на «% Выполнения» и формула расчета процента на \[Факт\] / \[План\] \* 100%.

3. Добавлена новая вкладка «Контроль». На ней отображены все документы, статьи которых не отражены в структуре отчета ОПиУ

   ![](./reliz-1-45-0-11.png){width=2602px height=496px}

### Исправление ошибок

1. Исправлена ошибка, когда при периодичности **Год** могло выводится несколько столбцов одного года.

2. Исправлена ошибка, когда при периодичности **Год** суммы в группировках считались некорректно.
...
1. Изменено визуальное оформление форм регистра и записи

## Поиск и замена по автоправилам

1. Добавлен новый отбор «Не проведенные документы»